...
access_token= "access-sandbox-f96ef0f8-f3a3-41ee-809c-b1051d5b0ff9"
...
@app.route("/api/create_payment_recipient",methods=["POST"])
...
@app.route("/api/create_payment_initiation",methods=["POST"])
def initiate_payment():
    data_json = request.get_json()
    pretty_print_response(data_json)
    recipient_id = data_json["recipient_id"]
    ...
    response = client.payment_initiation_payment_create(req).to_dict()
    pretty_print_response(response)
    payment_id = response["payment_id"]
    status = response["status"]
    return {"status":status,"payment_id":payment_id}
@app.route("/api/get_payment_initiation",methods=["POST"])
def get_payment_initiation():
    from plaid.model.payment_initiation_payment_get_request import PaymentInitiationPaymentGetRequest
    data = request.get_json()
    payment_id = data["payment_id"]
    req = PaymentInitiationPaymentGetRequest(payment_id=payment_id)
    response = client.payment_initiation_payment_get(req).to_dict()
    payment_id = response["payment_id"]
...
@app.route("/api/create_payment_consent",methods=["POST"])
def create_payment_consent():
    data_json = request.get_json()
    pretty_print_response(data_json)
    recipient_id = data_json["recipient_id"]
    req = PaymentInitiationConsentCreateRequest(
    recipient_id=recipient_id,
    reference="TestPaymentConsent",
    scopes= [PaymentInitiationConsentScope(value="ME_TO_ME")],
    constraints=PaymentInitiationConsentConstraints(
        valid_date_time=PaymentConsentValidDateTime(to=dt.datetime.strptime("2024-04-07T08:53:42.06717+02:00", "%Y-%m-%dT%H:%M:%S.%f%z")),
        max_payment_amount=PaymentConsentMaxPaymentAmount(currency=PaymentAmountCurrency("GBP"),value=15.00),
        periodic_amounts=[
        PaymentConsentPeriodicAmount(
            amount=PaymentConsentPeriodicAmountAmount(currency=PaymentAmountCurrency("GBP"),value=40.00),
    ...
    response = client.payment_initiation_consent_create(req).to_dict()
...
# TODO Frontend and backend Integrations
